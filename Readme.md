...
В своём рассчёте функция использует циклический сдвиг влево.
...
Казалось бы, функция похожа на предыдущую и должна давать тоже неплохое распределение, однако её эффективность сложно противопоставить RolHash. Это любопытно, но не является предметом данного исследования...

#### GnuHash

Распространённая функция хеширования, похожая на контрольную сумму, но использующая некоторые "магические" числа (33, 5381) в своём алгоритме.
...
Функция показала наилучшие результаты на данном массиве слов, не даром она так повсеместно используется.
