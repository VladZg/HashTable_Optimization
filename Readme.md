...
- Сравнить работу различных хэш-функций и выявить лучшую из них
...
Профилирование программы производится при помощи утилит ``callgrind``, ``valgrind`` и с использованием просмотрщика ``kcachegrind``, а профилирование по времени при помощи библиотеки ``<time.h>``.
...
Нетрудно видеть, что наименьшее значение дисперсии продемонстрировала функция GnuHash.
...
<kbd>
  <img src="./Pictures/callgrind_finally.png" width="500" height="350" />
...
Как мы видим, наиболее эффективной оказалась оптимизация, проделанная самим компилятором, однако наши старания тоже не прошли зря, ведь в результате удалось достичь ускорения в **3.21** раза.
...
- Пройдя длинный путь и применив 6 оптимизаций, основанных на разных принципах, удалось ускорить процесс поиска элемента в хеш-таблице в **3.21** раза, причём можно было бы достичь и большего абсолютного коэффицента ускорения при более неоптимизированной начальной версии программы

## Источники и литература

- Алгоритм функции хеширования CRC32: https://soltau.ru/index.php/themes/dev/item/461-kak-
- Сайт-зеркало с описанием intrinsic-инструкций процессоров *Intel*: https://www.laruence.com/sse/#expand=5939
- Калькулятор онлайн: https://calculator888.ru/
- Руководство по модулю ``re`` для регулярных выражений в *Python*: https://pythonru.com/osnovy/modul-re-dlja-reguljarnyh-vyrazhenij-v-python
- Описание структуры и принципов работы хеш-таблиц: https://neerc.ifmo.ru/wiki/index.php?title=%D0%A5%D0%B5%D1%88-%D1%82%D0%B0%D0%B1%D0%BB%D0%B8%D1%86%D0%B0
