...
Как ни удивительно, программа снова ускорилась (причём даже больше, чем при ассемблерной оптимизации функции GnuHash), а функция.
...
<kbd>
  <img src="./Pictures/asm_code_FindInHashTable.png" width="500" height="300" />

</kbd>

Однако как видно из ассемблерного листинга этой функции, приведённого выше, полученного с флагом компиляции ``-O3``, компилятор берёт задачу оптимизации алгоримта деления на себя и соперничать с ним в этом я не вижу смысла.
...
Нетрудно видеть, что по мере ускорения программы, новые решения по увеличению её производительности оказывают всё меньшее влияние. Так

Также для обоснования того, что поставленная цель работы была достигнута, приведу более полный скриншот из профилировщика, со всеми функциями, не включёнными в расмотрение в предыдущих пунктах:
...
Как мы видим, следующие строчки после уже знакомых нам функций занимают ``a`` и ``b`` - , затем идут ``c`` и ``d`` - ... . То есть в целом, остальные функции либо не вносят значимого вклада во время работы программы, либо не относятся к нашей задаче оптимизации поиска элемента в хеш-таблицы. Таким образом обосновывается завершение оптимизаций.
